...
static void initialize(void)
{
  ...
  trf1->Scale(2, 2, 1);
  auto tex1 = Texture::Make("face", "../../images/stopwatch.jpg");
  auto face = Node::Make(trf1, {Color::Make(1, 1, 1), tex1}, {Disk::Make()});
  auto trf2 = Transform::Make();
  trf2->Translate(5, 5, 0);
  auto trf3 = Transform::Make();
  trf3->Scale(0.1, 2, 1);
  auto tex2 = Texture::Make("face", glm::vec3(1, 1, 1));
  auto pointer = Node::Make(trf2, {Node::Make(trf3, {Color::Make(1, 0, 0), tex2}, {Triangle::Make()})});

  auto shader = Shader::Make();
  shader->AttachVertexShader("../../shaders/test3b/vertex.glsl");
  shader->AttachFragmentShader("../../shaders/test3b/fragment.glsl");
  shader->Link();
